...
The system SHALL provide an `arashi handoff` command that generates a Markdown handoff report for the current coordinated Arashi workspace without mutating repository state.

#### Scenario: Markdown report summarizes workspace state
...
- **WHEN** a user runs `arashi handoff` outside a configured Arashi workspace
- **THEN** Arashi exits non-zero
- **AND** the output explains that a configured Arashi workspace is required
