...
## Purpose
TBD - created by archiving change add-tab-launch-disposition. Update Purpose after archive.
## Requirements
...
#### Scenario: Terminal.app creates a true tab
- **WHEN** Terminal.app is selected with disposition `tab` and an exact current window/settings target is available
- **THEN** one static AppleScript transaction creates a tab in that captured window
- **AND** passes exact cwd and current shell as data rather than interpolating them into AppleScript source
- **AND** applies the captured settings profile to the created tab
...
#### Scenario: macOS tab target or version is unavailable
- **WHEN** Terminal.app, iTerm2, or macOS Ghostty is selected for `tab` without the required target or supported API version
