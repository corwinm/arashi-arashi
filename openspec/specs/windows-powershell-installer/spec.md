...
Arashi SHALL update Windows direct-binary installs by deferring the PowerShell installer until the current Arashi process can exit, and the deferred installer MUST apply the same verified four-file payload and rollback contract as a fresh install.
