# interactive-lifecycle-hook-input Specification

## Purpose
Define how trusted human `create` and `remove` invocations provide terminal input to lifecycle hooks while JSON, explicitly disabled, unavailable-terminal, and unresolved execution paths fail closed with immediate EOF across supported shells and platforms.
## Requirements
