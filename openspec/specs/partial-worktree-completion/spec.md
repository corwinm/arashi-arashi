...
The system SHALL preserve ordinary missing-repository clone behavior when the current workspace is not a coordinated worktree or when no usable local source repository can be resolved for a selected missing repository.
