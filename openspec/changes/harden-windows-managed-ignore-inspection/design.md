...
The fallback parser will first split on the final tab and require the returned path to equal the one safe managed path supplied as the direct argument. It will then enumerate every possible `:<decimal-line>:` boundary in the prefix and accept the source, line, and pattern only when exactly one candidate has nonempty source and pattern fields. A source or pattern containing delimiter-like text that makes multiple parses possible is unusable rather than guessed. The line-number field remains opaque because it is not exposed by Arashi.
