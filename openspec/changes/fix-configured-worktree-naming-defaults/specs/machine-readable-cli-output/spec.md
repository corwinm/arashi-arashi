...
Configured `create --json` success, dry-run, and destination-collision failure envelopes SHALL expose parent and child destinations from the same authoritative configured plan used by human output, collision preflight, and execution. Successful execution SHALL preserve the existing `data.repositories[]` record shape and report each destination at `data.repositories[].worktreePath`. Dry-run SHALL preserve `data.repositories` as the execution-result list and report prospective records at `data.dryRunOutcome.plannedWorktrees[]`, where every record contains `branchName`, `planStatus`, `repositoryName`, and `worktreePath`; `worktreePath` is an absolute string or `null` only when planning cannot resolve one. Destination collision SHALL use error code `WORKTREE_DESTINATION_COLLISION` and report exactly one conflict record at `error.details.conflict` with `repositoryName` and absolute `worktreePath`. No replacement destination field or duplicate plan is introduced. The destination values SHALL reflect `<canonical repository naming component>-<branch>` for a configured bare parent and `<branch>` for a configured non-bare parent beneath the effective base, while preserving the standard one-document envelope and stdout-isolation contracts.
