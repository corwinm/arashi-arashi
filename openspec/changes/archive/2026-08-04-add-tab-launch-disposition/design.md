...
| cmux | Existing create-and-focus of a dedicated workspace in the active cmux session, documented as the independent-session equivalent | The same create-and-focus workspace operation, explicitly treated as cmux's in-session vertical-tab equivalent |
...
The managed Kitty, tmux/sesh, and cmux rows intentionally allow the same underlying primitive for both requested dispositions because those primitives are explicitly defined as the selected multiplexer’s in-session tab-equivalent independent context. Herdr has a distinct tab-within-workspace API and therefore requires strict active-workspace evidence for `--tab`. Other abstractions remain unsupported where no stable tab target exists.

Managed context always outranks the containing terminal application for disposition routing. A Ghostty pane inside tmux creates a tmux window; a Ghostty pane inside Herdr creates a Herdr tab; and an invocation inside cmux creates a cmux workspace/vertical tab in that session. Native Ghostty tab creation is considered only when no higher-precedence managed context is selected.
