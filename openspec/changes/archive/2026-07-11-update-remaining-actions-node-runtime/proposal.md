...
- Inventory all workflows across the Arashi meta-repository and child repositories for actions that GitHub reports as targeting Node.js 20.
...
- Validate each affected repository independently and confirm representative completed GitHub Actions runs no longer emit the Node.js 20 deprecation annotation.
- Add or document GitHub Actions dependency automation coverage in the meta-repository and each child repository so future action runtime-major drift is surfaced earlier.
...
- Affected repositories: the `arashi-arashi` meta-repository plus `repos/arashi/`, `repos/arashi-docs/`, `repos/arashi-skills/`, and `repos/arashi-vscode/`. The meta-repository workflow was added after initial planning and was included when the inventory was refreshed before merge.
...
- Dependency automation configuration is added in the meta-repository and each child repository; none had GitHub Actions Dependabot coverage when updated.
