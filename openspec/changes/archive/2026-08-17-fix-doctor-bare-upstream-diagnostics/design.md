...
Introduce a read-only Git helper that inspects the current local branch, `branch.<name>.remote`, `branch.<name>.merge`, strict `@{upstream}` resolution, the expected `refs/remotes/<remote>/<merge-branch>` ref, and all `remote.<remote>.fetch` values. Doctor invokes it for a non-detached repository whose parsed `remoteBranch` is null, or whose generic status refresh failure can leave a configured-but-gone upstream label that would otherwise suppress topology diagnosis. A proven missing-remote-ref refresh warning retains precedence. When `branch.<name>.merge` has multiple values, the helper preserves empty entries and follows Git's effective first value rather than the last value returned by a single-value config lookup. Invalid empty effective values receive manual inspection guidance, and valid multi-valued configuration is not appended to by remediation. Dynamic Git operands are placed after option terminators where supported so option-shaped remote and branch names remain operands. The helper itself performs no fetch or Git mutation; it consumes the state available after doctor's existing `checkAllRepos()` status refresh, which remains outside this narrow classification change and can update remote-tracking refs.
...
When no existing fetch mapping targets the expected remote-tracking destination from another source, suggested commands form an ordered, copy-pasteable sequence that adds a branch-specific fetch mapping, fetches the configured remote, and sets the local branch upstream to `<remote>/<merge-branch>`.

When existing positive mappings conflict pairwise at any destination namespace, or one or more exact or wildcard mappings target that destination or a ref-namespace ancestor or descendant from another source, or map the configured merge source to another destination, the finding instead lists the conflicting configured refspecs and suggests only the read-only command that displays the remote's fetch mappings. Doctor does not recommend an automatic mutation, fetch, or upstream change for this ambiguous topology: replacing a wildcard could delete unrelated branch coverage, while adding another mapping can either make `git fetch` fail or leave Git resolving the merge source through the pre-existing noncanonical mapping. The user must resolve the conflict manually. Negative refspecs and valid source-only or empty-destination refspecs that match the configured merge source, plus malformed configured values, receive the same conservative manual-review treatment, even when another positive mapping appears to cover the branch, because a matching exclusion can suppress that source and an invalid configured refspec can make the subsequent fetch fail. Valid negative and exact source-only or empty-destination patterns that do not match the configured merge source remain unrelated and do not suppress otherwise safe remediation. Destinationless wildcard refspecs always require manual review because Git requires wildcard mappings on both sides. Structural parsing is supplemented with read-only `git check-ref-format` validation of exact or wildcard-substituted source and destination refnames so shaped-but-Git-invalid values do not receive automatic guidance. Raw configured values retain leading and trailing whitespace through validation and evidence collection; only Git's output record delimiter is removed, because trimming can fabricate a valid mapping that `git fetch` rejects. Exact destinations and wildcard destination namespaces that are ancestors or descendants of the expected tracking ref also require manual resolution because Git cannot store both a ref and a child beneath that ref.

On Windows, doctor cannot reliably infer whether its output will be pasted into PowerShell, Command Prompt, or Git Bash. Because those shells have incompatible escaping rules for valid Git-derived values, the topology-aware finding retains its structured evidence and explanation but emits no copy-paste command strings. The user applies equivalent commands in the active Windows shell rather than receiving a recommendation that is safe in one shell and unsafe or invalid in another.
...
- **Refspec matching can be subtle** → Parse positive fetch mappings conservatively, support exact and wildcard source/destination pairs needed by Git remote tracking, require non-overlapping wildcard prefix/suffix matches, and fall back to the generic finding when classification is uncertain.
...
- **Suggested configuration can affect user-owned Git state** → The new diagnostic path remains read-only, recommends an additive branch-specific mapping only when no destination conflict exists, and falls back to read-only inspection plus manual resolution for exact or wildcard destination conflicts; only a user who runs an unambiguous suggested mutation changes configuration.
