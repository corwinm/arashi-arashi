...
- GREEN: focused planner/coordinated-remove suite passed 20 tests, including real spawned-CLI single-document JSON coverage for dirty descendants, dependency-blocked partial failure, and post-hook plan invalidation.
- Final CLI validation after the last source edit: 129 test files passed, 1 skipped; 1,726 tests passed, 6 skipped; format, lint (0 errors), typecheck, completion check, build, and `git diff --check` passed.
