...
After onboarding, add creates one complete `RepoConfig` containing `path`, `gitUrl`, and selected canonical inline fields plus a separate immutable script plan. After final confirmation, it performs the existing expected-byte check under the config lock, revalidates every planned destination, prepares each complete no-op scaffold privately, and atomically installs it at the exact active path with no-replace semantics and runtime-ready permissions before calling `saveConfig` at most once. Installation MUST be anchored to opened, identity-checked directory handles and use no-follow traversal/no-replace installation for every parent and destination component; an implementation path that cannot prove those guarantees on the current platform fails closed before creating an active file. Parent identities are checked again through the anchored handles immediately after installation. Prompt collection and validation do not write config or script files, and a partial file is never visible at an active hook path.

Add's rollback ledger records the exact bytes, mode where meaningful, and identity of every script it created. If any script install, config save, or later add step fails, rollback removes only invocation-created scripts that remain byte-and-mode-identical regular files at the exact planned destinations; edited, replaced, symlinked, chmodded, or pre-existing paths are preserved and reported as incomplete cleanup. Config rollback still compares the complete entry, not only `path` and `gitUrl`. Existing canonical clone, coordinated worktree/branch, and managed-ignore cleanup ordering remains unchanged.
