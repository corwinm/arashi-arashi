...
- **AND** target repository/source path, parent path, `ARASHI_BRANCH_NAME`, scope, source, and workspace mode describe that same invocation
...
Arashi SHALL discover `.sh` lifecycle scripts on POSIX and `.ps1`, `.cmd`, or `.bat` lifecycle scripts on Windows. If multiple supported candidates exist for one scope and logical lifecycle, discovery MUST fail before hook execution or lifecycle mutation and identify every conflicting path.
...
Configured create, configured remove, and standalone lifecycle hooks SHALL use a default timeout of `300000` milliseconds. A configured positive finite `hooks.timeout` value SHALL override that default for every configured lifecycle hook.
...
Arashi SHALL test generated hook examples and activation instructions through real temporary configured and standalone command workflows rather than validating only file existence or manually injected operation keys.
