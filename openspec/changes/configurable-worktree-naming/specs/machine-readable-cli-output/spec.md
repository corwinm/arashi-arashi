...
- **WHEN** configured non-bare create for branch `feature/auth` is previewed in human and `--dry-run --json` modes under omitted policy or explicit `current` and `preserve`
