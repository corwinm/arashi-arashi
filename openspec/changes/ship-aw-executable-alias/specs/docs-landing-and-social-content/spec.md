...
Canonical user documentation SHALL identify `aw` as the supported “Arashi Workspace” executable alias, state that `arashi` remains canonical, and explain that supported installations provide both names with equivalent behavior.

#### Scenario: Visitor scans the landing page
- **WHEN** a visitor reviews the landing page or getting-started install outcome
- **THEN** concise copy states that commands may be run as canonical `arashi` or shorthand `aw`
...
- **THEN** the owning page accurately explains whether the channel creates both names, how `aw` expands, and that an unrelated pre-existing alias must be moved deliberately
