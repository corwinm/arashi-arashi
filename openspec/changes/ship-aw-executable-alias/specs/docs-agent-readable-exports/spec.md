...
Generated Markdown routes, `/llms.txt`, and `/llms-full.txt` SHALL identify `aw` as the supported “Arashi Workspace” executable alias, preserve `arashi` as canonical, and route agents to current installation and shell guidance without teaching a user-created shell alias as equivalent support.
