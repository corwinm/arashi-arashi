...
The release job will have a job-level `github.ref == 'refs/heads/main'` condition. GitHub evaluates that condition before allocating the runner or executing branch-controlled workflow steps, so a manual dispatch targeting another branch is skipped before checkout and before either repository secret can be exposed. Development validation may exercise the signing code with the actual secrets before this final guard is added, but the merged workflow must never import release secrets from non-default-branch code.
