...
function resolveSchema(schema: JsonObject, value: unknown): unknown {
  ...
  while (object(current) && typeof current.$ref === "string") {
    ...
    if (!reference.startsWith("#/definitions/") || seen.has(reference))
      return undefined;
    seen.add(reference);
...
function schemaSupportsMaterialization(schema: JsonObject): boolean {
  ...
  const repository = definitions
    ? resolveSchema(schema, definitions.RepoConfig)
    : undefined;
  if (
    !object(repository) ||
  ...
  const required = Array.isArray(repository.required)
    ? repository.required
    : [];
...
function guidanceSupportsMaterialization(content: string): boolean {
  ...
  const contains = (...values: string[]) =>
    values.every((value) => lower.includes(value));
  return (
    contains("repos.<name>.copy", "repos.<name>.symlink") &&
    /git[- ]primary/i.test(normalized) &&
    /same[^.]{0,100}relative path/i.test(normalized) &&
    /pre-create[\s\S]{0,260}copy[\s\S]{0,200}symlink[\s\S]{0,260}post-create/i.test(
      normalized,
    ) &&
    /--no-hooks[^.]{0,200}(?:does not|doesn't|never)[^.]{0,120}(?:disable|skip)[^.]{0,100}materialization/i.test(
      normalized,
    ) &&
    /missing[^.]{0,180}skip/i.test(normalized) &&
    /(?:no overwrite|without overwrite|never overwrites?)/i.test(normalized) &&
    /(?:no[^.]{0,100}copy[^.]{0,100}hard[- ]?link[^.]{0,100}junction[^.]{0,100}fallback|(?:never|does not|cannot)[^.]{0,100}fall(?:s)? back[^.]{0,100}copy[^.]{0,100}hard[- ]?link[^.]{0,100}junction)/i.test(
      normalized,
    ) &&
    ...
    /dry-run[^.]{0,180}(?:preview|plan)[^.]{0,180}(?:order|declaration)/i.test(
      normalized,
    ) &&
    /doctor[^.]{0,180}(?:non-mutating|without repair|without mutation|inspect|diagnos)/i.test(
      normalized,
    ) &&
    /(?:missing sources?[^.]{0,180}(?:visib(?:le|ly)|non-fatal|optional)[^.]{0,120}skip|missing sources?[^.]{0,180}skip[^.]{0,120}(?:visib(?:le|ly)|non-fatal|optional))/i.test(
      normalized,
    ) &&
    /destination[^.]{0,180}(?:inside|within|remain|never escape)[^.]{0,100}worktree/i.test(
      normalized,
    ) &&
    /(?:configured[^.]{0,120}only|standalone[^.]{0,120}(?:not supported|unsupported|not available))/i.test(
      normalized,
    )
  );
}
...
function isNegatedBefore(statement: string, index: number): boolean {
  ...
  const contrast =
    /(?:\b(?:yet|but|however|while|although|though|whereas|and|or)\b|;)/gi;
  let clauseStart = 0;
  for (const match of beforeAction.matchAll(contrast))
    clauseStart = (match.index ?? 0) + match[0].length;
  const prefix = statement
    .slice(Math.max(clauseStart, index - 80), index)
    .toLowerCase();
  return (
    /\b(?:does?|do|is|are|must|should|can|may|will)\s+not(?:\s+\w+){0,5}\s*$/.test(
      prefix,
    ) ||
...
export function contradictoryGuidance(content: string): boolean {
  for (const statement of semanticStatements(content)) {
    if (
      ...
      /materialization|repos\.<name>\.(?:copy|symlink)|copy[^.]{0,40}symlink/i.test(
        statement,
      ) &&
      hasAffirmativeAction(
        statement,
        /\b(?:available|supports?|supported|accepts?|loads?|uses?)\b/gi,
      )
    )
      return true;
    if (
    ...
    if (
      ...
      hasAffirmativeAction(
        statement,
        /\b(?:reads?|uses?|sources?)\b[^.]{0,120}\b(?:caller|active) checkout\b/gi,
      )
    )
      return true;
    if (
      /(?:symlink[^.]{0,100}before[^.]{0,100}copy|copy[^.]{0,100}after[^.]{0,100}symlink)/i.test(
        statement,
      )
    )
      return true;
    ...
    if (
      ...
      hasAffirmativeAction(
        statement,
        /\b(?:disables|skips)\s+(?:configured\s+|declarative\s+)?materialization/gi,
      )
    )
      return true;
    if (
      /symlink/i.test(statement) &&
      /independently mutable/i.test(statement) &&
      /\.env/i.test(statement) &&
      hasAffirmativeAction(
        statement,
        /\b(?:provides?|supports?|use)\b[^.]{0,120}independently mutable/gi,
      )
    )
      return true;
  }
...
async function readPackagedSkillGuidance(root: string): Promise<string> {
  const temporaryRoot = await mkdtemp(
    join(tmpdir(), "arashi-materialization-package-"),
  );
  ...
  try {
    ...
    if (contradictoryGuidance(content))
      throw new Error("canonical package contradicts guidance");
    return content;
  } finally {
...
async function check(
  root: string,
): Promise<{ diagnostics: Diagnostic[]; ok: boolean }> {
  const diagnostics: Diagnostic[] = [];
  try {
    const parsed: unknown = JSON.parse(
      await readFile(join(root, schemaSource), "utf8"),
    );
  ...
  for (const group of guidanceGroups) {
    try {
      ...
      if (
        !guidanceSupportsMaterialization(
          surfaces.map(({ content }) => content).join("\n"),
        ) ||
        ...
        add(
          diagnostics,
          group.category,
          "MATERIALIZATION_GUIDANCE_MISMATCH",
          group.source,
  ...
  try {
    if (
      !guidanceSupportsMaterialization(await readPackagedSkillGuidance(root))
    ) {
      add(
        diagnostics,
        "skills",
...
if (args.has("--json")) console.log(JSON.stringify(result, null, 2));
  ...
  console.log(
    "Worktree materialization contracts passed across CLI, docs, exports, and skills.",
  );
else {
